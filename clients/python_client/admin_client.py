import socket
import threading
import customtkinter as ctk
from tkinter import messagebox
import re
import sys

SERVER_IP = "127.0.0.1"
SERVER_PORT = 2000

if len(sys.argv) >= 2:
    try:
        SERVER_PORT = int(sys.argv[1])
        print(f"[CONFIG] Using port {SERVER_PORT} from arguments")
    except ValueError:
        print(f"[ERROR] Invalid port: {sys.argv[1]}")
        print(f"Usage: python {sys.argv[0]} [port]")
        sys.exit(1)

client = None
authenticated = False
user_role = None
username = None


def send_message(action, data):
    """Send a PTT-format message (space padding, no embedded NULs)."""
    action_padded = action.ljust(12)[:12]
    data_padded = data.ljust(150)[:150]
    message = f"PTT{action_padded}{data_padded}END"
    
    print(f"[SEND] Outgoing message ({len(message)} chars):")
    print(f"   Header: {message[:3]}")
    print(f"   Action: '{message[3:15]}'")
    print(f"   Data (first 30): '{message[15:45]}'")
    print(f"   Footer: {message[-3:]}")
    
    client.sendall(message.encode('utf-8'))

def parse_response(raw_data):
    """Parse server response bytes."""
    try:
        if b"PTT" in raw_data and b"END" in raw_data:
            ptt_idx = raw_data.index(b"PTT")
            end_idx = raw_data.index(b"END")
            
            action_start = ptt_idx + 3
            action_end = action_start + 12
            data_start = action_end
            data_end = end_idx
            
            action_bytes = raw_data[action_start:action_end]
            action = action_bytes.decode('utf-8', errors='ignore').strip().rstrip('\x00')
            
            data_bytes = raw_data[data_start:data_end]
            response_data = data_bytes.decode('utf-8', errors='ignore').strip().rstrip('\x00')
            
            print(f"[PARSE] OK — action='{action}', data='{response_data[:50]}'")
            return action, response_data
    except Exception as e:
        print(f"[ERROR] Parse error: {e}")
        import traceback
        traceback.print_exc()
    
    return None, None


def login(user, password):
    global authenticated, user_role, username, client
    
    try:
        client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        print(f"[CONNECT] Connecting to {SERVER_IP}:{SERVER_PORT}...")
        client.connect((SERVER_IP, SERVER_PORT))
        print("[CONNECT] Connected to server")
        
        login_data = f"{user};{password}"
        print(f"[LOGIN] Sending LOGIN: user='{user}'")
        send_message("LOGIN", login_data)
        
        print("[LOGIN] Waiting for server response...")
        response = client.recv(1024)
        print(f"[LOGIN] Received ({len(response)} bytes): {response[:100]}")
        
        action, data = parse_response(response)
        print(f"[LOGIN] Parsed — action='{action}', data='{data}'")
        
        if action == "OK" and "AUTH_OK" in data:
            authenticated = True
            username = user
            
            if "role=ADMIN" in data:
                user_role = "ADMIN"
            elif "role=OBSERVER" in data:
                user_role = "OBSERVER"
            
            print(f"[LOGIN] Login OK — role={user_role}")
            return True, user_role
        else:
            print(f"[LOGIN] Login failed — action={action}, data={data}")
            return False, None
            
    except Exception as e:
        print(f"[ERROR] Login error: {e}")
        import traceback
        traceback.print_exc()
        return False, None

def send_command(cmd):
    try:
        send_message("COMMAND", cmd)
        
        for _ in range(3):
            response = client.recv(1024)
            action, data = parse_response(response)
            
            if action == "DATA":
                continue
            
            if action == "OK":
                return True, "Command executed"
            elif action == "DENIED":
                return False, "Permission denied"
            else:
                return False, f"Unexpected response: {action}"
        
        return False, "No response for command"
    except Exception as e:
        return False, str(e)

def list_users():
    try:
        print("[LIST] Requesting user list...")
        send_message("LIST", "")
        
        for _ in range(3):
            response = client.recv(1024)
            action, data = parse_response(response)
            
            if action == "DATA":
                continue
            
            print(f"[LIST] LIST reply — action='{action}', data='{data}'")
            
            if action == "OK":
                users = []
                user_entries = data.split(',')
                
                for entry in user_entries:
                    entry = entry.strip()
                    if not entry or 'NONE' in entry:
                        continue
                    
                    parts = entry.split(':')
                    if len(parts) >= 3:
                        users.append({
                            'user': parts[0],
                            'role': parts[1],
                            'ip': parts[2],
                            'port': 'N/A'
                        })
                        print(f"  [USER] {parts[0]} - {parts[1]} - {parts[2]}")
                
                print(f"[LIST] Total users: {len(users)}")
                return users
            else:
                print(f"[LIST] Could not get list — action='{action}'")
        
        print("[LIST] No LIST reply after retries")
        return []
    except Exception as e:
        print(f"[ERROR] list_users: {e}")
        import traceback
        traceback.print_exc()
        return []


class AdminPanel:
    def __init__(self):
        self.root = ctk.CTk()
        self.root.title("NetDrive - Admin")
        self.root.geometry("700x600")
        self.root.resizable(False, False)
        
        ctk.set_appearance_mode("dark")
        ctk.set_default_color_theme("blue")
        
        self.show_login()
    
    def show_login(self):
        login_frame = ctk.CTkFrame(self.root)
        login_frame.pack(fill="both", expand=True, padx=50, pady=100)
        
        title = ctk.CTkLabel(login_frame, text="NetDrive Admin", font=("Verdana", 24, "bold"))
        title.pack(pady=20)
        
        ctk.CTkLabel(login_frame, text="Username:", font=("Verdana", 12)).pack(pady=5)
        self.username_entry = ctk.CTkEntry(login_frame, width=300)
        self.username_entry.pack(pady=5)
        self.username_entry.insert(0, "admin")
        
        ctk.CTkLabel(login_frame, text="Password:", font=("Verdana", 12)).pack(pady=5)
        self.password_entry = ctk.CTkEntry(login_frame, width=300, show="*")
        self.password_entry.pack(pady=5)
        self.password_entry.insert(0, "admin123")
        
        login_btn = ctk.CTkButton(login_frame, text="Sign in", command=self.do_login, width=300)
        login_btn.pack(pady=20)
        
        info_label = ctk.CTkLabel(login_frame, text="Demo: admin/admin123, observer/observer123", 
                                   font=("Verdana", 10), text_color="gray")
        info_label.pack(pady=10)
    
    def do_login(self):
        user = self.username_entry.get()
        password = self.password_entry.get()
        
        success, role = login(user, password)
        
        if success:
            if role != "ADMIN":
                messagebox.showerror("Error", "Only administrators can open this panel")
                return
            
            for widget in self.root.winfo_children():
                widget.destroy()
            
            self.show_admin_panel()
        else:
            messagebox.showerror("Error", "Invalid credentials")
    
    def show_admin_panel(self):
        header = ctk.CTkFrame(self.root, fg_color="#2C2C2C")
        header.pack(fill="x", padx=0, pady=0)
        
        title = ctk.CTkLabel(header, text=f"Admin — {username}", 
                             font=("Verdana", 20, "bold"))
        title.pack(pady=15)
        
        main_frame = ctk.CTkFrame(self.root)
        main_frame.pack(fill="both", expand=True, padx=20, pady=20)
        
        cmd_frame = ctk.CTkFrame(main_frame)
        cmd_frame.pack(fill="x", padx=10, pady=10)
        
        ctk.CTkLabel(cmd_frame, text="Vehicle control", 
                     font=("Verdana", 16, "bold")).pack(pady=10)
        
        buttons_grid = ctk.CTkFrame(cmd_frame)
        buttons_grid.pack(pady=10)
        
        commands = [
            ("Speed up", "SPEED UP"),
            ("Slow down", "SLOW DOWN"),
            ("Turn left", "TURN LEFT"),
            ("Turn right", "TURN RIGHT")
        ]
        
        for i, (label, cmd) in enumerate(commands):
            btn = ctk.CTkButton(buttons_grid, text=label, width=150,
                                command=lambda c=cmd: self.execute_command(c))
            btn.grid(row=i//2, column=i%2, padx=10, pady=5)
        
        users_frame = ctk.CTkFrame(main_frame)
        users_frame.pack(fill="both", expand=True, padx=10, pady=10)
        
        ctk.CTkLabel(users_frame, text="Connected users", 
                     font=("Verdana", 16, "bold")).pack(pady=10)
        
        self.users_text = ctk.CTkTextbox(users_frame, height=200)
        self.users_text.pack(fill="both", expand=True, padx=10, pady=10)
        
        refresh_btn = ctk.CTkButton(users_frame, text="Refresh list", 
                                     command=self.refresh_users)
        refresh_btn.pack(pady=10)
        
        self.refresh_users()
    
    def execute_command(self, cmd):
        success, message = send_command(cmd)
        
        if success:
            messagebox.showinfo("Success", f"Command '{cmd}' completed")
        else:
            messagebox.showerror("Error", f"Could not run command: {message}")
    
    def refresh_users(self):
        users = list_users()
        
        self.users_text.delete("1.0", "end")
        
        if users:
            self.users_text.insert("1.0", f"{'User':<20} {'Role':<15} {'IP':<20}\n")
            self.users_text.insert("end", "="*60 + "\n")
            
            for user in users:
                line = f"{user['user']:<20} {user['role']:<15} {user['ip']:<20}\n"
                self.users_text.insert("end", line)
        else:
            self.users_text.insert("1.0", "Could not load user list")
    
    def run(self):
        self.root.mainloop()

if __name__ == "__main__":
    app = AdminPanel()
    app.run()
